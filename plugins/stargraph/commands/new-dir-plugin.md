---
description: Scaffold a directory-based Stargraph plugin (drop-in folder under ~/.stargraph/plugins/) — skills, tools, packs, stores, plugin.toml
...
Drop-in plugin layout that `stargraph-dir-plugins` discovers from
`$STARGRAPH_PLUGINS_DIR` (default `~/.stargraph/plugins/`). No pip install
...
a tarball. Use `/stargraph:new-skill` (or the legacy plugin-package
...
3. **What does it ship?** Combine any of:
   - **Markdown skills** (`skills/<name>/SKILL.md`) — will call `/stargraph:new-md-skill` per skill
   - **Tools** (`tools/*.py`) — will call `/stargraph:new-tool` per tool
   - **Bosun packs** (`packs/<name>/`) — will call `/stargraph:new-pack` per pack
...
1. Creates `~/.stargraph/plugins/<name>/` (or `$STARGRAPH_PLUGINS_DIR/<name>/`).
...
4. Runs `stargraph plugins verify <path>` for offline validation.
5. Runs `stargraph plugins reload && stargraph plugins inspect <name>` to confirm
   the live `stargraph serve` registers it.

## Report

- Tree of the new dir-plugin.
- Output of `stargraph plugins verify` and `stargraph plugins inspect`.
