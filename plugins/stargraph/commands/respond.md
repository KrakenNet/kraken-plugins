---
description: Respond to a paused (HITL) Stargraph run — supply human input, approve/deny, resume
...
```bash
STATUS=$(curl -fsS "${STARGRAPH_URL}/v1/runs/${RID}" \
  -H "Authorization: Bearer ${STARGRAPH_TOKEN}" | jq -r .status)
...
- Stream URL to follow: `${STARGRAPH_URL}/v1/runs/${RID}/stream`
