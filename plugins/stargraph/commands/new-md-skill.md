---
description: Scaffold a Stargraph SKILL.md (lightweight markdown skill format that compiles to a typed stargraph.skills.Skill)
...
body) compiles into a typed `stargraph.skills.Skill` instance. Same runtime
...
instead of a full pip-installable package. Use `/stargraph:new-skill` for the
...
6. **Has a typed sub-graph?** (y/n; if yes, will call `/stargraph:new-graph`)
...
- Result of `stargraph skills compile <path>` validation.
