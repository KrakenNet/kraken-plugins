---
description: Scaffold + verify Stargraph graphs. Writes state.py, nodes/, rules/, stargraph.yaml, tests; runs stargraph graph verify; iterates.
...
5. Write `stargraph.yaml` per the schema in `references/graph-yaml-schema.md`.
6. Write `tests/test_graph.py` — runs the graph against fixture state, asserts at least one transition.
7. Run `uv run stargraph graph verify graphs/<graph_name>/stargraph.yaml`.
